...
function renderCartSidebar() {
    ...
    if (currentCart.length === 0) {
        cartItemsContainer.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:40px 0;">Your cart is empty.</p>';
...
scheduleForm.addEventListener('submit', (e) => {
    ...
    currentCart = [];
    updateCartUI();
    showToast(orderType === 'scheduled' ? 'Order scheduled!' : 'Order placed!');
    closeCart();
    openDashboard();
    scheduleForm.reset();
...
function renderDashboard() {
    ...
    if (sortedOrders.length === 0) {
        upcomingList.innerHTML = '<p style="color:var(--text-muted);">No orders yet.</p>';
        return;
    }
    sortedOrders.forEach(order => {
        ...
        let statusLabel = order.orderType === 'normal' ? 'Preparing' : 'Scheduled';
        upcomingList.innerHTML += `
            <div class="dash-item">
                ...
                <div class="dash-item-datetime">
                    <div>${fDate} at ${fTime}</div>
                    <button onclick="cancelOrder('${order.id}')" style="color:red; background:none; border:none; cursor:pointer;">Cancel</button>
                </div>
            </div>
        `;
    });
...
window.filterRestaurants = function(category) {
    ...
    filtered.forEach(rest => {
        restaurantsGrid.innerHTML += `
            <div class="restaurant-card" onclick="openRestaurant('${rest.id}')">
                <img src="${rest.cardImg}" alt="${rest.name}" class="card-img">
                <div class="card-info">
                    <div class="card-name">${rest.name}</div>
                </div>
            </div>
